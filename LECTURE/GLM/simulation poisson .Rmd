...
for(i in 1:nsim){
  ...
  ####데이터바꾸면서 돌리기####
  train <- generate_data(n,t,p,sigma_b,beta_true)
  
  for(j in 1:length(lambdas)){
    X_vars <- paste("X", 1:p, sep = "")
    formula <- as.formula(paste("y ~", paste(X_vars, collapse = " + ")))
    model <- glmmLasso(formula, rnd = list(group = ~ 1), family=poisson(),
...
mean(sort(MSE_tr[,1])[1:20])
mean(sort(MSE_tr[,2])[1:20])
mean(sort(MSE_tr[,3])[1:20])
mean(sort(MSE_tr[,4])[1:20])
mean(sort(MSE_tr[,5])[1:20])
mean(sort(MSE_tr[,6])[1:20])

sd(sort(MSE_tr[,1])[1:20])
sd(sort(MSE_tr[,2])[1:20])
sd(sort(MSE_tr[,3])[1:20])
sd(sort(MSE_tr[,4])[1:20])
sd(sort(MSE_tr[,5])[1:20])
sd(sort(MSE_tr[,6])[1:20])
```
```{r}
p = 500
n = 40
